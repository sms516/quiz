/*
 * Slim v2.1.0 - Image Cropping Made Easy
...
var urlToImage = function urlToImage(url) {
	var img = new Image();
	img.src = url;
	return img;
};

var imageToData = function imageToData(img) {
	var canvas;
	var ctx;
	var dataURL;
...
var revealElement = function revealElement(element, duration) {
	snabbt(element, {
		fromOpacity: 0,
		opacity: 1,
		duration: duration,
...
var getCanvasImageData = function getCanvasImageData(canvas) {
	return canvas.getContext('2d').getImageData(0, 0, canvas.width, canvas.height);
};

var cropCanvas = function cropCanvas(canvas, callback, rect) {
		...
		value: function _save() {
			...
			if (!this._options.service && !(this._isInitialising && !this._isImageOnly())) {
				...
				this._options.willSave.apply(this, [data, function (data) {
					...
					_this22._options.didSave.apply(_this22, [data]);
				}]);
			}
			...
			if (this._options.service && allowUpload) {
				...
				this._options.willSave.apply(this, [data, function (data) {
					...
					_this22._upload(data, function (err, res) {
						...
						// we did upload data
						_this22._options.didUpload.apply(_this22, [err, data, res]);
   ...
   */
		...
		value: function _doRemove() {
			...
			setTimeout(function () {
				...
				_this26._options.didRemove.apply(_this26, [data]);
		...
		value: function _doUpload(callback) {
			...
			this._hideButtons(function () {
				...
				_this27._save(function (err, data, res) {

					_this27._removeState('upload');
					_this27._stopProgress();

					if (callback) {
						callback.apply(_this27, [err, data, res]);
					}
...
(function () {
	...
	// the did callbacks
	['Init', 'Load', 'Save', 'Upload', 'Remove'].forEach(function (cb) {
