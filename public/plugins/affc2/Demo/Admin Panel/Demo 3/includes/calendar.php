	...
	**************************************************************************/
	...
	class calendar
	{
		...
		*/
		...
		{
			$query = sprintf('SELECT id, title, description, start, end, allDay, color, url, category, user_id FROM %s WHERE id = %d',
				  mysqli_real_escape_string($this->connection, $this->table),
				  mysqli_real_escape_string($this->connection, $id)
			);
		...
		*/
		...
		{	
			...
			if(isset($event['rep_id']) && strlen($event['rep_id']) !== 0)
			{
				...
				if($is_rep == true)
				{
					$process = $this->repetitive_event_procedure($event['allDay'], $start, $end, $event['rep_id'], $id, $event);
			...
			$query = sprintf('UPDATE %s 
						...
						',
										...
										mysqli_real_escape_string($this->connection, htmlspecialchars($event['category'])),
										mysqli_real_escape_string($this->connection, $event['allDay']),
										$id
